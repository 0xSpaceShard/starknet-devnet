...
If you're doing [L1-L2 message exchange](./postman), restarting will by default not affect Devnet's connection with L1 nor the L1->L2 message queue. The effect that L1-L2 messages may have had on Devnet before restarting shall be reverted, including any L2 contracts used for messaging, but calling [`flush`](./postman#flush) will not have new messages to read until they are actually sent. If you wish to re-process the already-seen L1->L2 messages when you restart, restart the message queues by setting the `restart_l1_l2_messaging` parameter shown below. If you set this flag, you will need to [reload the L1-side messaging contract](./postman#load). The L2->L1 message queue is restarted regardless of the flag.
...
JSON-RPC
{
    ...
    "params": {
        ...
        "restart_messages_to_l2": true | false
    }
}
```
