...
impl Starknet {
    pub fn new(config: &StarknetConfig) -> DevnetResult<Self> {
        ...
        let pending_state_diff = state.commit_diff()?;
    ...
    /// last tx, excluding declared classes; they are to be committed as part of the block update.
    /// Updates the pending_state_diff. Check `StarknetState::commit_diff` for more info.
    pub fn commit_diff(&mut self) -> DevnetResult<StateDiff> {
        let state_diff = self.pending_state.commit_diff()?;
