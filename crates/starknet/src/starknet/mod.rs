...
impl Starknet {
    ...
    pub fn get_block_txs_count(&self, block_id: &BlockId) -> Result<u64> {
        let block;
        if let Some(block_number) = self.blocks.block_number_from_block_id(block_id) {
            block = self.blocks.num_to_block.get(&block_number);
        } else {
            return Err(Error::NoBlock);
        }

        Ok(block.unwrap().get_transactions().len() as u64)
    }
}
