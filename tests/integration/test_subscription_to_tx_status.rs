...
async fn should_notify_only_when_tx_moved_from_pending_to_latest_block() {
    ...
    let subscription_id_before =
        subscribe_tx_status(&mut ws_before_tx, &expected_tx_hash, Some(block_tag)).await.unwrap();
    ...
    let subscription_id_after =
        subscribe_tx_status(&mut ws_after_tx, &expected_tx_hash, Some(block_tag)).await.unwrap();
...
async fn should_notify_pending_subscriber_if_tx_already_in_pending_block() {
    ...
    subscribe_tx_status(&mut ws_latest, &expected_tx_hash, Some(BlockId::Tag(BlockTag::Latest)))
        .await
        .unwrap();
...
async fn should_never_notify_if_block_id_newer_than_tx_addition() {
    ...
    subscribe_tx_status(&mut ws, &tx_hash, Some(BlockId::Hash(newer_block_hash))).await.unwrap();
...
async fn should_notify_when_tx_in_pending_if_legal_numeric_block_id() {
    ...
    let (address, mint_amount, expected_tx_hash) = first_mint_data();
    let subscription_id =
        subscribe_tx_status(&mut ws, &expected_tx_hash, Some(BlockId::Number(0))).await.unwrap();
